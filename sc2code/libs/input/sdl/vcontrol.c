...
#include SDL_INCLUDE(SDL_touch.h)
#include SDL_INCLUDE(SDL_gesture.h)
#include <stdlib.h>
#include <string.h>
#include <ctype.h>
#include "vcontrol.h"
...
key_init (void)
{
	...
	(void)SDL_GetKeyboardState (&num_sdl_keys);
...
{
    if (symbol & SDLK_SCANCODE_MASK) {  // WTF
        symbol &= ~SDLK_SCANCODE_MASK;
    }
...
VControl_HandleEvent (const SDL_Event *e)
{
	switch (e->type)
	{
        ...
        case SDL_FINGERDOWN:
            MouseButtonDown = 1;
            SDL_Touch *state = SDL_GetTouch(e->tfinger.touchId);
            MouseX = ((float)e->tfinger.x / state->xres) * ScreenWidth;
            MouseY = ((float)e->tfinger.y / state->yres) * ScreenHeight;
//			VControl_ProcessKeyDown (SDLK_RETURN);
			event_ready = 1;
            break;
        case SDL_FINGERUP:
            MouseButtonDown = 0;
//			VControl_ProcessKeyUp (SDLK_RETURN);
            break;
#endif
