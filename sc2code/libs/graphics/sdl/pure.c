...
static void TFB_Pure_XScaled_Preprocess (int force_full_redraw, int transition_amount, int fade_amount);
static void TFB_Pure_XScaled_Postprocess (void);
...
static TFB_GRAPHICS_BACKEND pure_xscaled_backend = {
...
TFB_Pure_ConfigureVideo (int driver, int flags, int width, int height)
{
	...
	{
		videomode_flags = SDL_SWSURFACE;
		ScreenWidthActual = 480;
		ScreenHeightActual = 320;
		graphics_backend = &pure_xscaled_backend;
	}
	else
	{
		videomode_flags = SDL_SWSURFACE;
