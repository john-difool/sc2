...
//	glEnd ();
    ...
    const GLfloat vertex_data[] = 
    {
        ...
        0, ScreenHeightActual,
    };
    
    const GLfloat texcoord_data[] = 
    {
        0, 0,
        (ScreenWidth / 512.0f) / 0.6f, 0,
        (ScreenWidth / 512.0f) / 0.6f, (ScreenHeight / 256.0f) / 1.0f,
        0, (ScreenHeight / 256.0f) / 1.0f,
    };
...
TFB_GL_Preprocess (int force_full_redraw, int transition_amount, int fade_amount)
{
	...
	glLoadIdentity ();
	glOrthof (0,ScreenWidthActual,ScreenHeightActual, 0, -1, 1);
...
//					GL_Screens[screen].updated.x));
        int sub_x = GL_Screens[screen].updated.x;
        int sub_y = GL_Screens[screen].updated.y;
        int sub_width = GL_Screens[screen].updated.w;
        int sub_height = GL_Screens[screen].updated.h;
        
		glTexImage2D( GL_TEXTURE_2D, 0, GL_RGBA, ScreenWidth, ScreenHeight, 0, GL_RGBA, GL_UNSIGNED_BYTE, NULL );
		int y;
        for( y = 0; y < sub_height; y++ )
        {	
            void *row = (Uint32 *)SDL_Screens[screen]->pixels + (( y + sub_y ) * ScreenWidth + sub_x);
            glTexSubImage2D( GL_TEXTURE_2D, 0, /* xoffset */ sub_x, /* yoffset */ ( y + sub_y), sub_width, 1, GL_RGBA, GL_UNSIGNED_BYTE, (const GLvoid *)row );
        }
...
//				GL_Screens[screen].updated.x * 2));
        int sub_x = GL_Screens[screen].updated.x * 2;
        int sub_y = GL_Screens[screen].updated.y * 2;
        int sub_width = GL_Screens[screen].updated.h * 2;
        int sub_height = GL_Screens[screen].updated.w * 2;
        int img_width = ScreenWidth * 2;
        glTexImage2D( GL_TEXTURE_2D, 0, GL_RGBA, sub_width, sub_height, 0, GL_RGBA, GL_UNSIGNED_BYTE, NULL );
        
        for( int y = 0; y < sub_height; y++ )
        {
            char *row = GL_Screens[screen].scaled->pixels + ( ( y + sub_y ) * img_width + sub_x ) * 4;
            glTexSubImage2D( GL_TEXTURE_2D, 0, 0, y, sub_width, 1, GL_RGBA, GL_UNSIGNED_BYTE, (const GLvoid *)row );
        }
